...
#include "llvm/IR/ConstrainedOps.def"
  ...
  case Intrinsic::ptroffset: {
    ...
    setValue(&I, DAG.getNode(ISD::SRL, getCurSDLoc(), DestVT, Ptr,
  ...
  case Intrinsic::ptrunmask: {
    ...
    setValue(&I, DAG.getNode(ISD::OR, getCurSDLoc(), DestVT, Ptr,
                             DAG.getZExtOrTrunc(Const, getCurSDLoc(), DestVT)));
    return;
  }
  case Intrinsic::ptrunmask1: {
    SDValue Ptr = getValue(I.getOperand(0));
    SDValue Const = getValue(I.getOperand(1));
	...
	case Intrinsic::safe_store: {
    SDValue Dst = getValue(I.getArgOperand(0));
    SDValue Val = getValue(I.getArgOperand(1));
    SDValue Root = getMemoryRoot();
    SDValue Ret = DAG.getSafeStore(Root, sdl, Dst, Val,
                                   MachinePointerInfo(I.getArgOperand(0)));
		DAG.setRoot(Ret);
    setValue(&I, Ret);
    return;
